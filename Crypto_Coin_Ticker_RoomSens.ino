...
// last edit:
// added optional support for the SHT30 Temperature and Humidity sensor built in the official grey vertical base 
// it makes the info panel a bit larger so only select if you want it. -> when using the M5Burner just select regular or the sensor version (Celcius or Fahrenheit) 
// For regular users without the vertical base i recommend to use the usual version 1.0.4 because you would loose some pixels from the candle chart for no benefit.
//----------------------------------------------------------------------------------------------------------------------------
